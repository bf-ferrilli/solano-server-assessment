...
function setupEditButtons() {
  ...
  rows.forEach(row => {
    // Create edit button column if it doesn't exist
    if (row.cells.length === 9) { // If there are exactly 9 columns (without edit)
...
function toggleEditMode(row) {
  ...
  const roleCell = row.cells[2];
  const osCell = row.cells[3]; // Assuming OS is in column 4
  const editCell = row.cells[9]; // The edit button cell
