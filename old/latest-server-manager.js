// server-manager.js - With detailed OS lifecycle information support and row deletion/addition
...
function resetAllEditing() {
  ...
  if (currentEditingRow) {
    ...
    // Restore edit button
    const editCell = currentEditingRow.cells[9];
    if (editCell) {
      editCell.innerHTML = '';
...
function setupEditButtons() {
  ...
  rows.forEach(row => {
    ...
    if (row.cells.length === 9) { // If there are exactly 9 columns (without actions)
      const actionsCell = document.createElement('td');
  ...
  if (headerRow && headerRow.cells.length === 9) {
...
function toggleEditMode(row) {
  ...
  const roleCell = row.cells[2];
  const osCell = row.cells[3]; // Assuming OS is in column 4
  const actionsCell = row.cells[9]; // The actions cell
